...
const ALL_CRYPTO = [
  { date: "21-08-2022 6:19 PM", user: "Kyla watson", type: "Deposit", amount: "0.00030", fees: "0.00000", total: "+0.00030", currency: "LTCTEST", status: "Success" },
  { date: "20-08-2022 6:19 PM", user: "-", type: "Withdrawal", amount: "0.00030", fees: "0.00000", total: "-0.00034", currency: "LTCTEST", status: "Success" },
  { date: "20-08-2022 6:18 PM", user: "Irish watson", type: "Sent", amount: "0.00030", fees: "0.00000", total: "-0.00034", currency: "LTCTEST", status: "Success" },
  { date: "20-08-2022 6:04 PM", user: "Brynn Floyd", type: "Received", amount: "0.00030", fees: "0.00000", total: "+0.00030", currency: "BTC", status: "Success" },
  { date: "20-08-2022 5:50 PM", user: "-", type: "Sent", amount: "0.00200", fees: "0.00000", total: "-0.00212", currency: "DOGETEST", status: "Success" },
];

export default function AdminAllCryptoPage() {
  const [search, setSearch] = useState("");

  const filtered = ALL_CRYPTO.filter(t =>
    `${t.user} ${t.currency} ${t.type}`.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleExportCSV = () => {
    const headers = ["Date", "User", "Type", "Amount", "Fees", "Total", "Currency", "Status"];
    const rows = filtered.map(t => [
      t.date, t.user, t.type, t.amount, t.fees, t.total, t.currency, t.status
    ]);
    exportToCSV("all_crypto_transactions", headers, rows);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-[#1a1b26] border border-white/5 rounded-xl overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 border-b border-white/5 gap-4">
          <h2 className="text-lg font-bold text-white">All Crypto Transactions</h2>
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map((t, i) => (
                <tr key={i} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-5 py-3 text-primary cursor-pointer hover:underline whitespace-nowrap">{t.user}</td>
                  <td className="px-5 py-3 text-gray-300">{t.type}</td>
                  <td className="px-5 py-3 text-gray-300 font-mono">{t.amount}</td>
                  <td className="px-5 py-3 text-gray-300 font-mono">{t.fees}</td>
                  <td className={`px-5 py-3 font-mono ${t.total.startsWith('+') ? 'text-green-400' : 'text-red-400'}`}>{t.total}</td>
              ...
              <div className="border-t border-white/5 pt-4 mt-2">
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-base font-bold pt-2 border-t border-white/5">
                    ...
                    <span className={`${transactionToView.total.startsWith('+') ? 'text-green-400' : 'text-red-400'} font-mono`}>{transactionToView.total}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {isEditOpen && transactionToEdit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1a1b26] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between p-6 border-b border-white/5">
              <h2 className="text-lg font-bold text-white">Edit Crypto Transaction</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1a1b26] border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="p-6 text-center space-y-4">
              ...
              <p className="text-sm text-gray-400">Are you sure you want to delete this crypto transaction? This action cannot be undone.</p>
            </div>
