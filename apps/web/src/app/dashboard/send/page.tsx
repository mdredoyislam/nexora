...
export default function SendMoneyPage() {
  ...
  const fee = 0; // Fee (0.00%+0.00) Total Fee: 0.00

  const renderStepIndicator = () => {
    return (
      <div className="flex flex-col items-center justify-center mb-10">
        <h2 className="text-2xl font-bold mb-2 uppercase tracking-wide">SEND MONEY</h2>
  ...
  const renderStep1 = () => (
    <div className="max-w-xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        <p className="text-xs text-gray-500 -mt-4">Fee (0.00%+0.00) Total Fee: {fee.toFixed(2)}</p>
  ...
  const renderStep2 = () => (
    <div className="max-w-xl mx-auto">
      <p className="text-center text-gray-400 mb-8">
        Check your transfer information before confirmation.
      </p>
      ...
      <div className="space-y-4 mb-8 bg-[#161722] p-6 rounded-xl border border-white/5">
        ...
        <div className="flex justify-between items-center py-3 border-b border-white/5">
          ...
          <span className="font-bold">{currency} {amount || "0.00"}</span>
        </div>
        <div className="flex justify-between items-center py-3 border-b border-white/5">
        ...
        <div className="flex justify-between items-center py-3 border-b border-white/5">
          ...
          <span className="font-bold text-lg text-primary">{currency} {(parseFloat(amount || "0") + fee).toFixed(2)}</span>
        </div>
        {note && (
          <div className="py-3">
      ...
      <div className="space-y-4">
        <Button onClick={() => setStep(3)} className="w-full h-12 text-base flex items-center justify-center gap-2">
          Confirm Transfer <ChevronRight className="w-4 h-4" />
        </Button>
    ...
    <div className="max-w-md mx-auto text-center space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-400 text-sm">
          Money has been successfully sent to <span className="text-white">{email}</span>.
        </p>
      </div>

      <div className="bg-[#1a1b26] border border-white/10 rounded-xl p-8">
        <p className="text-primary text-sm font-medium mb-2">Sent Amount</p>
